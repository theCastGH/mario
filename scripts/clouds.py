...
class Clouds:
    def __init__(self, cloud_imges, count=16) -> None:
        self.clouds = []

        for i in range(count):
            self.clouds.append(Cloud((random.random() * 99999, random.random() * 99999), random.choice(cloud_imges), random.random() * 0.05 + 0.05, random.random() * 0.6+0.2))
